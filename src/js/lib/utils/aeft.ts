...
const getLatestFile = (dir: string, suffix: string): string | null => {
    ...
    fs.readdirSync(dir)
        ...
        .map((file) => {
            if (latestFile === null || getModified(path.join(dir, file)) > getModified(path.join(dir, latestFile))) {
                latestFile = file;
            }
        });
    return latestFile;
...
export const getPrefsDir = (): string => {
    ...
    const prefsDir = path.join(mainDir, 'Adobe', 'After Effects', parseFloat(appVersion).toFixed(1).toString());
    return prefsDir;
};
